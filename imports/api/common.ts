...
import { array, ObjectSchema, string } from "yup";
...
export const subscribeAll = <T extends Document>(collection: Mongo.Collection<T>, roles?: Role[]) => {
  const result = new TypedSubscribe({
    ...
    run: (options?: Mongo.Options<T>) => collection.find({ deletedAtDate: { $exists: false } }, options),
  });
  return result.fetch.bind(result);
};

export const subscribeSome = <T extends Document>(collection: Mongo.Collection<T>, roles?: Role[]) => {
  const result = new TypedSubscribe({
    ...
    run: (ids: string[], options?: Mongo.Options<T>) => collection.find({ _id: { $in: ids } } as any, options),
  });
  return result.fetch.bind(result);
};

export const subscribeOne = <T extends Document>(collection: Mongo.Collection<T>, roles?: Role[]) => {
  const result = new TypedSubscribe({
    ...
    run: (id: string, options?: Mongo.Options<T>) => collection.findOne({ _id: id } as any, options),
  });
  return result.fetch.bind(result);
};
